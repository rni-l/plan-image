...
export function ProductInfoTab({
    ...
    async (files: FileList | null) => {
      ...
      try {
        for (const file of Array.from(files)) {
          const asset = await api.upload<ProductAsset>(
        ...
        toast.error(
          "上传失败，请检查文件格式（JPEG / PNG / WEBP，最大 20 MB）",
        );
      } finally {
        setUploading(false);
      }
  ...
  async function handleExtract() {
    ...
    try {
      const result = await api.post<ExtractResult>(
        `/products/${productId}/extract-info`,
        {
          rawText: extractRaw,
        },
      );
        ...
        <div className="flex min-h-0 flex-col gap-6 overflow-y-auto pr-1">
          ...
          <div>
            ...
            <div className="flex flex-col gap-1.5">
              {specs.length === 0 && (
                <p className="text-xs text-zinc-400">暂无规格参数</p>
              )}
              {specs.map((spec, i) => (
          ...
          <div>
            ...
            <div className="flex flex-col gap-1.5">
              {points.length === 0 && (
                <p className="text-xs text-zinc-400">暂无卖点</p>
              )}
              {points.map((pt, i) => (
                <div key={i} className="flex items-center gap-2">
                  <Input
...
function SortableImageItem({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="group relative aspect-square overflow-hidden rounded-lg border border-zinc-100 bg-zinc-50"
    >
      <img
        src={imgUrl}
        alt=""
      ...
      <div
        {...listeners}
        {...attributes}
        className="absolute left-1 top-1 cursor-grab rounded bg-white/80 p-0.5 opacity-0 shadow-sm transition-opacity group-hover:opacity-100"
      >
      ...
      <button
        ...
        className="absolute left-1 bottom-1 rounded bg-white/80 p-0.5 opacity-0 shadow-sm transition-opacity hover:bg-white group-hover:opacity-100"
        aria-label="放大查看"
      >
      ...
      <button
        onClick={onDelete}
        className="absolute right-1 top-1 rounded bg-white/80 p-0.5 opacity-0 shadow-sm transition-opacity hover:bg-red-50 group-hover:opacity-100"
      >
...
function UploadCard({
  ...
  return (
    <button
      ...
      disabled={uploading}
      className="flex aspect-square flex-col items-center justify-center gap-1.5 rounded-lg border border-dashed border-zinc-200 bg-zinc-50 text-zinc-400 transition-colors hover:border-zinc-300 hover:bg-zinc-100 disabled:opacity-50"
    >
      {uploading ? (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        ...
        {/* Input area — shown until extraction succeeds */}
        {!extracted && (
          <div className="flex flex-col gap-3">
            <p className="text-xs text-zinc-500">
              粘贴产品描述、规格参数、宣传文案等原始文字，AI
              将自动提取规格参数、核心卖点和备注。
            </p>
            <Textarea
              rows={8}
              placeholder="粘贴原始文字内容…"
              value={rawText}
        ...
        {extracted && (
          <div className="flex flex-col gap-4 text-sm">
            ...
            {extracted.sellingPoints.length > 0 && (
              <div>
                <p className="mb-1.5 font-medium text-zinc-700">
                  核心卖点（{extracted.sellingPoints.length} 条）
                </p>
            ...
            {extracted.notes && (
              <div>
                ...
                <p className="rounded-md border border-zinc-100 bg-zinc-50 px-3 py-2 text-xs text-zinc-700 whitespace-pre-wrap">
                  {extracted.notes}
                </p>
              </div>
            )}
